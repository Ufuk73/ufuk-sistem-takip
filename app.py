...
def init_db():
  ...
  # Eski tabloları temizleyip sıfırdan kuruyoruz
  ...
  # Tabloları oluştur
  cursor.execute(
      "CREATE TABLE IF NOT EXISTS sistemler (id INTEGER PRIMARY KEY"
      " AUTOINCREMENT, bolge TEXT, sistem_adi TEXT NOT NULL, durum TEXT,"
      " aciklama TEXT, tarih TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
  )

  cursor.execute(
      "CREATE TABLE IF NOT EXISTS parcalar (id INTEGER PRIMARY KEY"
      " AUTOINCREMENT, sistem_id INTEGER, parca_adi TEXT NOT NULL, seri_no"
      " TEXT, durum TEXT, notlar TEXT, FOREIGN KEY(sistem_id) REFERENCES"
      " sistemler(id))"
  )

  cursor.execute(
      "CREATE TABLE IF NOT EXISTS notlar (id INTEGER PRIMARY KEY"
      " AUTOINCREMENT, tarih TIMESTAMP DEFAULT CURRENT_TIMESTAMP, baslik TEXT,"
      " icerik TEXT)"
  )
...
with tab2:
  ...
  if not df_sis.empty:
    ...
    df_parcalar = pd.read_sql_query(
        ...
        " FROM parcalar p LEFT JOIN sistemler s ON p.sistem_id = s.id",
        conn,
    )
